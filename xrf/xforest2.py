    ...
    def fmp_2qbf_enc(self, inst, feat_t):
        ...
        if 'enc' not in dir(self):
            if 'f' not in dir(self):
                self.f = Forest(self.cls, self.fnames)
            self.enc = SATEncoder(self.f, self.feature_names, len(self.class_names), self.fnames)
            cnf_t, rf_t = self.enc.qbf_encode_t(np.array(inst))
            cnf_RF, rf_0 = self.enc.qbf_encode_rf(np.array(inst))
        ...
        for f, intvs in six.iteritems(self.enc.ivars):
            ...
            for k, ele in enumerate(intvs):
                ...
                if int(f[1:]) == feat_t:
                    # we assume the local copy of selv_t is free
                    cnf_t.append([-j for j in tmp] + [intvs[k]])
                else:
                    cnf_t.append([selv] + [-j for j in tmp] + [intvs[k]])
        ...
        for i, (inp, val) in enumerate(zip(inps, inst), 1):
            if len(self.enc.intvs[inp]):
                ...
                for j, p in enumerate(self.enc.ivars_rf[inp]):
                    cl = [-selv]
                    if j == self.enc.imaps[inp][v]:
                        cl += [p]
                        str1 = self.enc.nameVar(sel2v[selv])
                        str2 = self.enc.nameVar(p)
                        assert str1 == str2.rstrip("^RF")
                        cnf_RF.append(cl)
                    else:
                        cl += [-p]
        ...
        return assums, univ_q, exist, cnf
...
class SATEncoder(object):
    ...
    def qbf_encode_t(self, sample):
        """
            Encoding the t-th RF, the prediction of this RF remain intact.
        """
        ##################################################
        ...
        def _compute_intervals():
            ...
            for feat, intvs in six.iteritems(self.intvs):
                ...
                for i, ub in enumerate(intvs):
                    self.imaps[feat][ub] = i
                    ivar = self.newVar(f'{feat}_intv{i}')
                    self.ivars[feat].append(ivar)
                    if ub != math.inf:
                        thvar = self.newVar(f'{feat}_th{i}')
                        self.thvars[feat].append(thvar)
        ...
        self.cmaj = self.forest.predict_inst(sample)
        if self.num_class == 2:
            # to keep the prediction of the t-th RF:
        ...
        ret_cnf = CNF()
        # define a variable for each clause
        rf_t = self.newVar(f'rf_t_cls')
        tmp_rf_t = []
        for idx, cls in enumerate(cnf_t):
            rf_t_cls = self.newVar(f'rf_t_cls_{idx}')
            tmp_rf_t.append(rf_t_cls)
            ret_cnf.append(cls + [-rf_t_cls])
            for lit in cls:
                ret_cnf.append([-lit, rf_t_cls])
        # define a variable for the whole CNF
        ret_cnf.append([-ele for ele in tmp_rf_t] + [rf_t])
        for ele in tmp_rf_t:
            ret_cnf.append([-rf_t, ele])
    ...
    def qbf_encode_rf(self, sample):
        """
            ...
            the prediction of this RF can change.
        """
        self.ivars_rf, self.thvars_rf = {}, {}
        ##################################################
        ...
        def _traverse(tree, k, clause):
            ...
            if tree.children:
                ...
                if f in self.intvs:
                    d = self.imaps[f][v]
                    pos, neg = self.thvars_rf[f][d], -self.thvars_rf[f][d]
                else:
                    assert False, "_traverse: this might be problematic"
        ...
        def _compute_intervals():
            ...
            for feat, intvs in six.iteritems(self.intvs):
                ...
                for i, ub in enumerate(intvs):
                    ...
                    ivar_rf = self.newVar(f'{feat}_intv{i}^RF')
                    self.ivars_rf[feat].append(ivar_rf)
                    if ub != math.inf:
                        assert len(self.thvars[feat])
                        thvar_rf = self.newVar(f'{feat}_th{i}^RF')
                        self.thvars_rf[feat].append(thvar_rf)
        ...
        self.forest.predict_inst(sample)
        # define Tautology var
        # 2022/07/18: consider binary classification first
        vtaut = self.newVar('Tautology')
        cnf_rf.append([vtaut])

        # introducing class-tree variables
        ctvars_rf = [[] for _ in range(num_tree)]
        ########## t-th copy of RF ##########
        for k in range(num_tree):
            for j in range(self.num_class):
                var = self.newVar(f'class{j}_tr{k}^RF')
                ctvars_rf[k].append(var)
        ...
        for k, tree in enumerate(self.forest.trees):
            ...
            _traverse(tree, k, [])
            # exactly one class var is true
            card = CardEnc.atmost(lits=ctvars_rf[k], vpool=self.vpool, encoding=EncType.cardnetwrk)
        ...
        if self.num_class == 2:
            ...
            if self.cmaj == 1 and not num_tree % 2:
                rhs = math.floor(num_tree / 2)
            lhs = [ctvars_rf[k][1 - self.cmaj] for k in range(num_tree)]
            atls = CardEnc.atleast(lits=lhs, bound=rhs, vpool=self.vpool, encoding=EncType.cardnetwrk)
            cnf_rf.extend(atls)
        else:
            zvars = []
            zvars.append([self.newVar('z_0_{0}^RF'.format(k)) for k in range(num_tree)])
            zvars.append([self.newVar('z_1_{0}^RF'.format(k)) for k in range(num_tree)])
            rhs = num_tree
            lhs0 = zvars[0] + [- ctvars_rf[k][self.cmaj] for k in range(num_tree)]
            atls = CardEnc.atleast(lits=lhs0, bound=rhs, vpool=self.vpool, encoding=EncType.cardnetwrk)
            cnf_rf.extend(atls)
            rhs = num_tree + 1
            lhs1 = zvars[1] + [- ctvars_rf[k][self.cmaj] for k in range(num_tree)]
            atls = CardEnc.atleast(lits=lhs1, bound=rhs, vpool=self.vpool, encoding=EncType.cardnetwrk)
            cnf_rf.extend(atls)
            pvars = [self.newVar('p_{0}^RF'.format(k)) for k in range(self.num_class + 1)]
            for k, p in enumerate(pvars):
                for i in range(num_tree):
                    if k == 0:
                        z = zvars[0][i]
                        cnf_rf.append([-p, z, -vtaut])
                    elif k == self.cmaj + 1:
                        z = zvars[1][i]
                        cnf_rf.append([-p, z, -vtaut])
                    else:
                        z = zvars[0][i] if (k < self.cmaj + 1) else zvars[1][i]
                        cnf_rf.append([-p, -z, ctvars_rf[i][k - 1]])
                        cnf_rf.append([-p, z, -ctvars_rf[i][k - 1]])
            cnf_rf.append([-pvars[0], -pvars[self.cmaj + 1]])
            lhs1 = pvars[:(self.cmaj + 1)]
            eqls = CardEnc.equals(lits=lhs1, bound=1, vpool=self.vpool, encoding=EncType.cardnetwrk)
            cnf_rf.extend(eqls)
        ...
        # lits of intervals
        ########## t-th copy of RF ##########
        for f, intvs in six.iteritems(self.ivars_rf):
            if not len(intvs):
                continue
            cnf_rf.append(intvs)
            card = CardEnc.atmost(lits=intvs, vpool=self.vpool, encoding=EncType.cardnetwrk)
            cnf_rf.extend(card.clauses)
        for f, threshold in six.iteritems(self.thvars_rf):
            for j, thvar in enumerate(threshold):
                d = j + 1
                pos, neg = self.ivars_rf[f][d:], self.ivars_rf[f][:d]
                if j == 0:
                    assert (len(neg) == 1)
        ...
        ret_cnf = CNF()
        # define a variable for each clause
        rf_0 = self.newVar(f'rf_0_cls')
        tmp_rf_0 = []
        for idx, cls in enumerate(cnf_rf):
            rf_0_cls = self.newVar(f'rf_0_cls_{idx}')
            tmp_rf_0.append(rf_0_cls)
            ret_cnf.append(cls + [-rf_0_cls])
            for lit in cls:
                ret_cnf.append([-lit, rf_0_cls])
        # define a variable for the whole CNF
        ret_cnf.append([-ele for ele in tmp_rf_0] + [rf_0])
        for ele in tmp_rf_0:
            ret_cnf.append([-rf_0, ele])
