...
# -*- coding:utf-8 -*-
#
#   Reproduce experiments (2QBF) alternative QBF encoding (\forall \exists)
#
################################################################################
import sys
import pickle
# xforest2 implements the alternative encoding
from xrf.xforest2 import XRF, Dataset
import os, time
...
def rf_fmp(rf_name, data_in, model_in, insts_file, feats_file, qbf_slv):
    ...
    for i, s in enumerate(inst_lines):
        ...
        qbf_waxp = []
        qbf_axp = []
        univ_q1, univ_q2, exist_q, claus = rf_md.fmp_2qbf_enc(inst, f_id)
        if univ_q1 is None:
            ...
            # rf_md.enc.printLits(univ_q1)
            # rf_md.enc.printLits(univ_q2)
            ...
            if qbf_slv == "depqbf":
                ...
                with open('qbf_solution.tmp', 'r') as f_qbf:
                    ...
                    if solution_lines[-1].startswith("UNSAT"):
                        # note that we use double negation
                        fmp_yes = True
    ...
    with open('results/rf_fmp_2qbf_alt_enc.txt', 'a') as f:
        f.write(exp_results)
    return
...
if __name__ == '__main__':
    args = sys.argv[1:]
    # example: python3 experiment-qbf2.py -bench pmlb_qbf.txt 20 caqe
